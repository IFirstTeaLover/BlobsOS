...
title.textContent = "BlobsDesktop App Store";
...
if (response.ok) {
    ...
    if (Array.isArray(list)) {
        ...
        for (const appArray of list) {
            ...
            if (appArray.icon) {
                const response = await fetch("https://raw.githubusercontent.com/IFirstTeaLover/BlobsOS/main/BlobsDesktop/AppStore/" + appName + "/icon.svg");
                if (response.ok) {
                    appIconSrc = await response.text();
                } else {
                    await blobsAPI.error("Failed to fetch app icon!")
                }
            }
            ...
            appDiv.append(desc);
            const ver = await blobsAPI.safeStorageRead(appName + "/version.txt")
            ...
            if (await blobsAPI.getFile("/home/applications/" + appName + ".js") && !update) {
                installState = true
                installText.textContent = "Uninstall";
            } else {
                installState = false;
                if (update && await blobsAPI.getFile("/home/applications/" + appName + ".js")) {
...
;
            ...
            installButton.onclick = async () => {
                if (installState === false) {
                    ...
                    const response = await fetch("https://raw.githubusercontent.com/ifirsttealover/BlobsOS/main/BlobsDesktop/AppStore/" + appName + "/main.js");
                    if (response.ok) {
                        installText.textContent = "Installing...";
                        for (const file of appArray?.extrafiles) {
                            const fileresponse = await fetch("https://raw.githubusercontent.com/ifirsttealover/BlobsOS/main/BlobsDesktop/AppStore/" + appName + "/" + file);
                            if (fileresponse.ok) {
                                await blobsAPI.writeFile(`/system/applicationStorage/${appArray.name}/${file}`, "file", fileresponse.text());
                            }
                        }
                        const code = await response.text();
                        await blobsAPI.writeFile("/home/applications/" + appName + ".js", "file", code);
                        await blobsAPI.safeStorageWrite(appName + "/version.txt", "file", appArray.version)
                        installText.textContent = "Uninstall";
                        iconSrc = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x-icon lucide-x"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>';
                        setAttrs(installStateIcon, {src: "data:image/svg+xml;utf8," + encodeURIComponent(iconSrc)});
                        if (appIconSrc) {
                            await blobsAPI.writeFile("/home/applications/" + appName + ".icon", "file", appIconSrc);
                        }
                        installState = true;
                    } else {
                        installText.textContent = "Failed to install!";
                        await blobsAPI.error("Failed to install '" + appName + "'. Status: " + response.status);
                    }
                } else if (installState === true) {
                    installState = "mid";
                    await blobsAPI.deleteFile("/home/applications/" + appName + ".js");
                    await blobsAPI.deleteFile("/system/applicationStorage/" + appName);
                    installText.textContent = "Install";
                    iconSrc = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-arrow-down-to-line-icon lucide-arrow-down-to-line"><path d="M12 17V3"/><path d="m6 11 6 6 6-6"/><path d="M19 21H5"/></svg>';
                    setAttrs(installStateIcon, {src: "data:image/svg+xml;utf8," + encodeURIComponent(iconSrc)});
                    installState = false;

...
if (!file || !file.endsWith(".json")) {
    ...
    projectData = JSON.parse(await blobsAPI.getFile(file));
    loadProj(file);
}
async function mainMenu() {
    await blobsAPI.setTitle("HuopaEngine Hub");
    ...
    const files = JSON.parse(await blobsAPI.getFile("/home/projects"));
    if (!files) return;
    for (const proj of files) {
        ...
        await setAttrs(projDiv, {
            ...
            "onclick":async() => {
                await blobsAPI.runApp("/home/applications/HuopaEngine.js", proj + "/project.json");
            }
        });
        body.append(projDiv);
    };
...
async function createProject() {
    if (!await blobsAPI.getFile("/home/projects")) await blobsAPI.writeFile("/home/projects", "dir", "[]");
    const name = await blobsAPI.openSaveDialog("/home/projects/myproject");
    const parent = name.split("/").slice(0, -1).join("/");
    if (!blobsAPI.getFile(parent)) { blobsAPI.createNotification("Invalid parent directory!", "Choose a path that has an existing parent directory."); mainMenu(); return; }
    projectData = 
{
    "objects":[
        {
    ...
    await blobsAPI.writeFile(`${name}/project.json`, "file", JSON.stringify(projectData, null, "    "));
...
async function loadProj(projPath) {
    ...
    blobsAPI.setMinWindowSize(725, 300);
    const projName = projPath.split("/").slice(0, -1).pop().replace(".json", "");
    await blobsAPI.setTitle(`HuopaEngine - ${projName}`);
    let save = false;
    const imageCache = {};
    body.innerHTML = "";
        ...
        while (true) {
            ...
            const handler = async (e) => {
                if (e.code === "KeyS" && (e.altKey || e.metaKey || e.ctrlKey)) {
                    ...
                    await blobsAPI.writeFile(projPath, "file", JSON.stringify(projectData));
    ...
    async function addImgToCache(path) {
        ...
        return new Promise(async (resolve) => {
            const img = new Image();
            img.onload = () => resolve();
            const file = await blobsAPI.getFile(`${projPath.split("/").slice(0, -1).join("/")}/${path}`);
            img.src = file;
            imageCache[path] = img;
    ...
    async function compileApp(build = false) {
        ...
        const embedFileAmount = JSON.parse(await blobsAPI.getFile(parent)).length;
        const parentFiles = JSON.parse(await blobsAPI.getFile(parent));
        let files = {};
        if (projectData.preferences.embedFiles && build) {
            ...
            for (let i = 0; i < fileAmount; i++) {
                ...
                files[parentFiles[i].replace(parent + "/","")] = await blobsAPI.getFile(parentFiles[i]);
            }
        }
        let code = `
// Setup
...
await blobsAPI.setTitle(${JSON.stringify(projName)});
let canvasCenterX;
let canvasCenterY;
const canvasContainer = document.createElement("div");
const projectCanvas = document.createElement("canvas");
...
if (projectData?.preferences?.hideTitlebar) await blobsAPI.removeTitlebar();
const opentypeEl = document.createElement("script");
await setAttrs(opentypeEl, {
    "src":"https://cdn.jsdelivr.net/npm/opentype.js@latest/dist/opentype.min.js"
});
...
async function addImgToCache(path) {
    ...
    return new Promise(async (resolve) => {
        ...
        if (embedFiles) {
            ...
            file = await blobsAPI.applicationStorageRead(path);
        } else {
            file = await blobsAPI.getFile(\`\${projPath.split("/").slice(0, -1).join("/")}/\${path}\`)
        }
    ...
    if (scripts) {
        ...
        for (const scriptPath of scripts) {
            ...
            if (embedFiles) {
                ...
                data = await blobsAPI.applicationStorageRead(scriptPath)
            } else {
                data = await blobsAPI.getFile(\`\${parent}/\${scriptPath}\`);
            }
            if (!data) continue;
            const gameScene = () => {
                return {
        ...
        `;
        if (build) {
            const path = await blobsAPI.openSaveDialog("/home/applications/" + projName + ".js");
            const parent = (projPath.split("/").slice(0, -1).join("/"))
            if (!await blobsAPI.getFile(parent)) { blobsAPI.createNotification("Invalid build location!", "Choose a path with a valid parent directory!"); return; }
            if (!projectData.preferences.embedFiles) {
                for (const file of JSON.parse(await blobsAPI.getFile(parent))) {
                    const filename = file.replace(projPath.split("/").slice(0, -1).join("/") + "/", "");
                    if (filename === "project.json") continue;
                    await blobsAPI.writeFile(`/system/applicationStorage/${path.split("/").pop().replace(".js","")}/${filename}`, "file", await blobsAPI.getFile(parent + `/${filename}`));
                }
            }
            
            await blobsAPI.writeFile(path, "file", code);
            await blobsAPI.runApp(path);
            blobsAPI.createNotification("App compiled successfully!", "The app can be found at: " + path);
        } else {
            await blobsAPI.deleteFile("/system/temp/HuopaEngine");
            const path = `/system/temp/HuopaEngine/${crypto.randomUUID().slice(0, 8)}`;
            const parent = (projPath.split("/").slice(0, -1).join("/"))
            for (const file of JSON.parse(await blobsAPI.getFile(parent))) {
                const filename = file.replace(projPath.split("/").slice(0, -1).join("/") + "/", "");
                if (filename === "project.json") continue;
                await blobsAPI.writeFile(`${path}/${filename}`, "file", await blobsAPI.getFile(parent + `/${filename}`));
            }
            await blobsAPI.writeFile(`${path}/main.js`, "file", code);
            await blobsAPI.runApp(`${path}/main.js`);
        }

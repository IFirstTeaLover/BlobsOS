const elevated = await blobsAPI.requestElevation();
if (!elevated) {
    await blobsAPI.createNotification("TilingWinManager", "You have to run as elevated!");
    await blobsAPI.closeApp();
}
blobsAPI.setWindowSize("0px", "0px")
blobsAPI.removeTitlebar();
blobsAPI.setWindowBlur("0");
blobsAPI.setWindowColor("rgba(0,0,0,0)", "rgba(0,0,0,0)");
blobsAPI.setWindowPosition("1px", "1px");
console.log("hi")
...
async function processFetch () {
    while (true) {
        const parsed = JSON.parse(await blobsAPI.getProcesses());
    ...
    while (true) {
        ...
        if (total !== 0) {
            ...
            for (let i = 0; i < total; i++) {
                ...
                blobsAPI.setProcessWindowPosition(win, left, top);
                blobsAPI.setProcessWindowSize(win, width, height);
                blobsAPI.setProcessWindowAnimation(win, `opacity 0.15s ease, transform 0.15s ease, width ${config.transition} ease, height ${config.transition} ease, left ${config.transition} ease, top ${config.transition} ease`);
            }

function setWindowDataLoop() {
    blobsAPI.setWindowPosition("0", "0");
    blobsAPI.setWindowSize("calc(100%)", "calc(100% - 2px)");
    setTimeout(setWindowDataLoop, 250);
}
blobsAPI.setWindowColor("#00000000", "#00000000");
blobsAPI.setWindowBlur("0px");
setWindowDataLoop()
...
async function renderDesktop() {
    let desktopData = await blobsAPI.getFile("/home/desktop");
    if (desktopData !== oldDesktopData) {
        if (!desktopData) {
            desktopData = "[]";
            await blobsAPI.writeFile("/home/desktop", "dir", "[]");
        }
        ...
        desktopDiv.style = "display: inline: width: 100%; height: 100%; display: flex; flex-wrap: wrap; flex-direction: row; padding: 0; align-content: flex-start;"
        ...
        for (const file of desktopData) {
            ...
            setAttrs(fileIcon, {src: "data:image/svg+xml;utf8," + encodeURIComponent(fileIconSrc)})
;
            ...
            fileDiv.onclick = async() => {
                if (fileSelected === file) {
                    ...
                    if (openInCode && await blobsAPI.getFile("/home/applications/Code.js")) {
                        blobsAPI.runApp("/home/applications/Code.js", file);
                    } else if (fileDir) {
                        blobsAPI.runApp("/home/applications/File Manager.js", file);
                    } else {
                        blobsAPI.runApp("/home/applications/Preview.js", file);
                    }
                } else {
                    const selectedFiles = document.querySelectorAll(".selected");
                    for (const el of selectedFiles) {
    ...
    oldDesktopData = await blobsAPI.getFile("/home/desktop");

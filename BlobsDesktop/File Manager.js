...
let returnId;
let favouriteFolders = await blobsAPI.applicationStorageRead("favouriteFolders.json");
if (!favouriteFolders) { await blobsAPI.applicationStorageWrite("favouriteFolders.json", "file", "[]"); favouriteFolders = '["/", "/system", "/home/applications", "/home/downloads"]'; }
...
if (typeof loadParams === "object" && loadParams.mode === "fileSelector") {
    await blobsAPI.setTitle("File Selector - Choose a file");
...
async function renderFileList(path) {
    blobsAPI.setTitle("File Manager - " + path)
    if (fileListDiv) {
        topBarList.remove()
        fileListDiv.remove()
        sideBarList.remove()
        ...
        await ContextMenu.set(fileDiv, [
            {
                ...
                "function":async() => {   
                    ...
                    await blobsAPI.applicationStorageWrite("favouriteFolders.json", "file", JSON.stringify(favouriteFolders));
                    renderFileList(path);
                }
            }
        ])
    ...
    if (!fileSelectorMode) {
        deleteButton.onclick = async() => {
                if (!pathSelected) {
                    let corePaths = await blobsAPI.getFile("/system/manifest.json");
                    if (corePaths) {
                        corePaths = JSON.parse(corePaths).corePaths
                        if (corePaths.includes(path)) {
                            await blobsAPI.warn("User tried deleting protected path!");
                            return;
                        }
                    }
                    await blobsAPI.deleteFile(path)
                    let parentPath = path.split("/").slice(0, -1).join("/");
                    if (!parentPath) parentPath = "/";
                    renderFileList(parentPath);
                } else {
                    await blobsAPI.deleteFile(pathSelected)
    ...
    if (await isDir(path) && !match) {
        let perms = true;
        let fileList = await blobsAPI.getFile(path);
            ...
            await ContextMenu.set(fileDiv, [
                {
                    name:"Delete",
                    "function":async() => {   
                        await blobsAPI.deleteFile(file)
                        renderFileList(path);
                    }
                }
            ])
                ...
                fileDiv.onclick = async () => {
                    if (pathSelected === file) {
                        ...
                        if (file.endsWith(".js") && notDir) {
                            if (fileSelectorMode) {
                                await blobsAPI.returnToHost(returnId, file);
                                await blobsAPI.closeApp();
                                return;
                            }
                            await blobsAPI.runApp(file);
                        } else if (notDir) {
                            if (fileSelectorMode) {
                                await blobsAPI.returnToHost(returnId, file); 
                                await blobsAPI.closeApp();
                            ...
                            if (openInCode && await blobsAPI.getFile("/home/applications/Code.js")) {
                                await blobsAPI.runApp("/home/applications/Code.js", file);
                            } else {
                                if (elevated) {
                                    await blobsAPI.runApp("/home/applications/Preview.js", file, true);
                                    return;
                                }
                                await blobsAPI.runApp("/home/applications/Preview.js", file);
                            }
        ...
        await blobsAPI.runApp("/home/applications/Preview.js", path);
    }
}

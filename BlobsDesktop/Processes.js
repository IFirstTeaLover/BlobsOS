...
if (elevated) blobsAPI.setTitle("Processes [Elevated]")
let procListDiv;
let oldProcList;
...
setAttrs(createProcessButton, {
    ...
    "onclick":async() => {
        ...
        input.addEventListener("keydown", async(e) => {
            if (e.key === "Enter") {
                const exists = await blobsAPI.getFile(input.value)
                if (input.value.length > 0 && exists) {
                    popup.remove();
                    if (elevatedCheckbox.checked) {
                        blobsAPI.runApp(input.value, undefined, true);
                    } else {
                        blobsAPI.runApp(input.value, undefined, false);
                    }
...
async function loadProcs() {
    const procIds = JSON.parse(await blobsAPI.getProcesses())[0];
    const procInfo = JSON.parse(await blobsAPI.getProcesses())[1];

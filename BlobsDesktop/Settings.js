...
async function newTab(flex, name, icon, mainScreenDiv, func) {
    ...
    tabIcon.style.width = "2em";
    tabIcon.style.height = "2em";
    tabIcon.style.filter = "brightness(0) invert(1)"
    tabLabel.textContent = name;

    tab.append(tabIcon, tabLabel);

    tab.style = `flex: 0 0 ${flex}%;
                 height: calc(50% - 0.75%);
...
async function wallpapersTabLoad() {
    ...
    for await (const wallpaperPath of wallpaperList) {
        ...
        const fileContent = await blobsAPI.getFile(wallpaperPath);
        ...
        if (ext === "mp4") {
            ...
            preview.src = await blobsAPI.compressImage(fileContent, 200, 112.5, "1");
        }
        ...
        wallpaperButton.onclick = async () => {
            await blobsAPI.writeFile("/system/env/systemconfig/settings/customization/wallpaperchosen.txt", "file", wallpaperPath.replace("/preview", ""));
        };
    }
    ...
    importButton.onclick = async () => {
        path = await blobsAPI.openFileDialog();
        const file = await blobsAPI.getFile(path)
        if (file) {
            await blobsAPI.writeFile("/system/env/wallpapers/" + path.split("/").pop(), "file", file);
            mainScreenDiv.remove()
...
async function customizationTabLoad() {
    ...
    slider.min = "0";
    slider.max = "12";
    slider.value = await blobsAPI.getFile("/system/env/systemconfig/settings/customization/bgblur.txt") || "3.5";
    slider.step = "0.1";

    const label = document.createElement("span");
    label.textContent = await blobsAPI.getFile("/system/env/systemconfig/settings/customization/bgblur.txt") + "px" || "3.5px";
    ...
    slider.oninput = async () => {
        ...
        await blobsAPI.writeFile("/system/env/systemconfig/settings/customization/bgblur.txt", "file", slider.value);
    };
    ...
    opacSlider.value = await blobsAPI.getFile("/system/env/systemconfig/settings/customization/bgopac.txt") || "0.90";

    const opacLabel = document.createElement("span");
    opacLabel.textContent = Math.round(await blobsAPI.getFile("/system/env/systemconfig/settings/customization/bgopac.txt") * 100) + "%";
    ...
    opacSlider.oninput = async () => {
        ...
        await blobsAPI.writeFile("/system/env/systemconfig/settings/customization/bgopac.txt", "file", opacSlider.value)
    };
...
async function dockTabLoad() {
    await blobsAPI.setTitle("Settings — Dock");
    const mainScreenDiv = document.createElement("div");
    const title = document.createElement("h1");
    ...
    opacSlider.max = "1";
    opacSlider.step = "0.05";
    opacSlider.value = await blobsAPI.getFile("/system/env/systemconfig/settings/customization/dockopac.txt");

    const opacLabel = document.createElement("span");
    opacLabel.textContent = Math.round(await blobsAPI.getFile("/system/env/systemconfig/settings/customization/dockopac.txt") * 100) + "%";
    opacLabel.style = "color: white; display: block; text-align: center; margin: 0.5em auto; padding-bottom: 1.5em;";
    opacSlider.margin = "0.5em auto";
    opacSlider.style.display = "block"
        ;
    ...
    opacSlider.oninput = async () => {
        opacLabel.textContent = Math.round(opacSlider.value * 100) + "%";
        await blobsAPI.writeFile("/system/env/systemconfig/settings/customization/dockopac.txt", "file", opacSlider.value)
    };
...
async function appTabLoad() {
    await blobsAPI.setTitle("Settings — Boot Apps");
    const mainScreenDiv = document.createElement("div");
    const title = document.createElement("h1");
    ...
    importButton.onclick = async () => {
        ...
        const file = await blobsAPI.getFile(path)
        if (file) {
            await blobsAPI.writeFile("/system/bootapps/" + path.split("/").pop(), "file", file);
            mainScreenDiv.remove()
            await appTabLoad()
        }
    };

    let bootAppList = await blobsAPI.getFile("/system/bootapps");
    if (!bootAppList) {
        bootAppList = "[]";
        await blobsAPI.writeFile("/system/bootapps", "dir", "[]")
    }
...
async function themeTabLoad() {
    await blobsAPI.setTitle("Settings — Themes");
    const mainScreenDiv = document.createElement("div");
    const title = document.createElement("h1");
    ...
    const response = await fetch("https://raw.githubusercontent.com/IFirstTeaLover/BlobsOS/main/BlobsDesktop/Themes/ThemeList.json");
    if (!response.ok) {
    ...
    for (const themeName of themeList) {
        ...
        const fileResponse = await fetch("https://raw.githubusercontent.com/IFirstTeaLover/BlobsOS/main/BlobsDesktop/Themes/" + themeName + ".css");

        preview.src = "https://raw.githubusercontent.com/IFirstTeaLover/BlobsOS/main/BlobsDesktop/Themes/" + themeName + ".png";
        themeButton.append(preview);
        themeListDiv.append(themeButton);
        const style = await fileResponse.text()
        themeButton.onclick = async () => {
            if (fileResponse.ok) {
                await blobsAPI.writeFile("/system/env/systemStyles.css", "file", style);
                await blobsAPI.requestSystemReboot()
            } else {
                const warning = document.createElement("h2");

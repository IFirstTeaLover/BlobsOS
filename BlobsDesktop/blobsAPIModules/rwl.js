...
return {
    render: async(code) => {
        const rwlSrc = await blobsAPI.getFile("/system/env/moduleSrc/rwlSrc.js");
        ...
        lastWindowWidth = await blobsAPI.getRenderedSize(rootEl, "width");
        lastWindowHeight = await blobsAPI.getRenderedSize(rootEl, "height");
    }
}
...
async function renderElemSetup(rawData) {
    if (rawData) {
        ...
        await blobsAPI.log(JSON.stringify(root.data.content.elements));
        await renderElems(root.data.content.elements, rootEl);
        rootEl.style.opacity = "1"
;
        
    } else {
        await blobsAPI.warn("No data returned from rwlSrc!")
    }
}
...
;
                ...
                if (key === "frame" || key === "section") {
                    ...
                    if (attrType === "padding") {
                        ...
                        if (paddingSanitize.padding) {
                            ...
                            await blobsAPI.warn("RWL.js Interpretor: Invalid padding value, ignored.");
                        }
        ...
        if (!yPointer.includes("%")) {
            ...
            if (anchorY === "t" || anchorY === "c") {
                ...
                const rootHeight = parseFloat(await blobsAPI.getRenderedSize(parentId, "height"));
...
;
        ...
        const elHeight = await blobsAPI.getRenderedSize(childEl, "height");
        const rootHeight = await blobsAPI.getRenderedSize(parentId, "height");
        ...
        if (!isNaN(yPointerNum) && !isNaN(totalHeight) && !isNaN(rootHeight)) {
            ...
            await blobsAPI.warn("RWL.js Interpreter: One or more dimensions could not be calculated.");
        }
...
async function resolvePosition(value, axis, parentId) {
    ...
    if (value.startsWith("calc(")) {
        ...
        if (!parts) {
            await blobsAPI.warn(`resolvePosition: Invalid calc expression "${value}"`);
            return NaN;
        }

        const rootSize = await blobsAPI.getRenderedSize(parentId, axis);
